...
module are `go` and `goSync` functions. They accept a function to execute, and additionally `go` accepts an optional
...
- ```ts
  type GoResultSuccess<T> = { data: T; success: true; error: undefined };
  ```
- ```ts
  type GoResultError<E extends Error = Error> = { data: undefined; error: E; success: false };
  ```
- ```ts
  type GoResult<T, E extends Error = Error> = GoResultSuccess<T> | GoResultError<E>;
  ```
- ```ts
...
Take a look at the [implementation](./index.ts) and [tests](./index.test.ts) for detailed examples and usage.
